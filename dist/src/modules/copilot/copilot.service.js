...
function normalizeIntensity(score) {
    return Math.max(0, Math.min(100, Math.round(score)));
}
function clusterFeedbackBoost(cluster, feedbacks) {
    const radius = Math.max(cluster.radius, 120);
    const nearbyFeedback = feedbacks.filter((feedback) => {
        const distance = haversineDistance({ lat: feedback.latitude, lng: feedback.longitude }, cluster.center);
        return distance <= radius;
    });
    if (!nearbyFeedback.length)
        return 1;
...
async function getCopilot(lat, lng) {
    ...
    const clusters = groupNearbyPoints(flows.map((flow) => ({ lat: flow.latitude, lng: flow.longitude, flowId: flow.id })), 200);
